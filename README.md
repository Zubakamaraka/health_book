...
кнопки. Показатели записываются кнопками «−» и «+», а не набором цифр на клавиатуре:
попасть в «36,6» на телефоне сложнее, чем кажется. Весь интерфейс масштабируется
кнопками «А−» и «А+» без потери чёткости.

- **назначения**: что, в какой дозе, с какой даты, для чего назначено и кем — с отметками приёма утром, днём, вечером и ночью
- **библиотека**: 177 лекарств и 37 процедур со справочными сведениями, поиск по действующему веществу (набрали «Найз» — нашёлся нимесулид и все аналоги)
- **свои позиции**: любое лекарство или процедуру можно добавить самому, со всеми полями
...
- **несколько профилей** — если приложением пользуются двое
...
| Кнопки «−» и «+», а не клавиатура | 214 позиций, поиск и разделы | Факты, справка, поиск в аптеках |

| Отчёт для врача | Тёмная тема | Крупный шрифт |
| --- | --- | --- |
| ![Отчёт для врача](docs/img/report.png) | ![Тёмная тема](docs/img/dark.png) | ![Крупный шрифт](docs/img/big-text.png) |
| Печать и сохранение в PDF | Переключается кнопкой вверху | Масштаб до 200% без потери чёткости |
...
data/library.json         встроенный словарь: 177 лекарств, 37 процедур
